...
import 'package:app_movil_dincydet/pages/manage_docs/doc_transmitted_dialogs.dart';
...
class DocumentTransmittedProvider extends ChangeNotifier {
  DocumentTransmittedProvider() {
  ...
  void onTapAdd() async {
    bool? complete = await showDialog(
        ...
        builder: (context) {
          return DocumentTransmittedNewDialog();
        });
  ...
  Future<void> onTapSearch() async {
    final data = await apiDocumentsTransmittedList(
  ...
  void onTapView(int index) async {
    bool? asigned = await showDialog(
        context: navigatorKey.currentContext!,
        builder: (context) {
          return DocumentTransmittedViewDialog(
  ...
  void onTapEdit(int i) async {
    ...
    if (document['FILE'] != null) {
      ...
      file = File('${tempDir.path}/${document['FILE'].split('/').last}');
      file.create();
    }
    bool? complete = await showDialog(
        context: navigatorKey.currentContext!,
        builder: (context) {
          return DocumentTransmittedNewDialog.edit(
